...
function safeParse<T>(
  Schema: { assert: (x: unknown) => T },
  value: unknown,
): { success: true; data: T } | { success: false } {
  try {
    return { success: true, data: Schema.assert(value) };
  } catch {
...
export class UnifiedTranslator {
  githubToAcp(raw: unknown): AcpEnvelope[] {
    const parsed = safeParse(GithubJsonRpcEnvelopeSchema, raw);
    if (!parsed.success) {
      return [this.buildProtocolError("github_invalid_message", -32600, "Invalid GitHub payload")];
    }
    const normalized = NormalizedInboundSchema.assert(this.normalizeGithubInbound(parsed.data));
    return this.normalizedToAcp(normalized);
  }

  claudeToAcp(raw: unknown): AcpEnvelope[] {
    const parsed = safeParse(ClaudeMessageSchema, raw);
    if (!parsed.success) {
      return [this.buildProtocolError("claude_invalid_message", -32600, "Invalid Claude payload")];
    }
    const normalized = NormalizedInboundSchema.assert(this.normalizeClaudeInbound(parsed.data));
    return this.normalizedToAcp(normalized);
  }

  acpToGithub(raw: unknown): unknown[] {
    const parsed = safeParse(AcpEnvelopeSchema, raw);
    if (!parsed.success) {
      return [this.githubErrorEnvelope("acp_invalid", -32600, "Invalid ACP payload from child agent")];
    }
    const normalized = NormalizedOutboundSchema.assert(this.normalizeAcpOutbound(parsed.data));
  ...
  private normalizedToAcp(normalized: NormalizedInbound): AcpEnvelope[] {
    switch (normalized.kind) {
      case "prompt":
        return [
          AcpEnvelopeSchema.assert({
            jsonrpc: "2.0",
            id: normalized.requestId ?? Date.now(),
            method: "session/prompt",
            params: {
              sessionId: normalized.sessionId,
              prompt: [{ type: "text", text: normalized.text ?? "" }],
              _meta: {
                ...(normalized.meta ?? {}),
      ...
      case "cancel":
        return [
          AcpEnvelopeSchema.assert({
            ...
            params: { sessionId: normalized.sessionId },
          }),
        ];
      case "permission_decision":
        return [
          AcpEnvelopeSchema.assert({
            ...
            params: {
              sessionId: normalized.sessionId,
              toolCall: { toolCallId: normalized.permissionId ?? "permission" },
              options: [],
              _meta: { outcome: normalized.decision },
            },
          }),
        ];
      case "tool_use":
          ...
          AcpEnvelopeSchema.assert({
            ...
            params: {
              ...
              update: {
                ...
                kind: "other",
                status: "pending",
                _meta: { command: normalized.command },
              },
            },
          }),
        ];
          ...
          AcpEnvelopeSchema.assert({
            ...
            params: {
              ...
              update: {
                ...
                content: { type: "text", text: "", _meta: { usage: normalized.usage } },
              },
            },
          }),
        ];
  ...
  private normalizeGithubInbound(
    payload: typeof GithubJsonRpcEnvelopeSchema.infer,
  ): NormalizedInbound {
  ...
  private normalizeClaudeInbound(payload: typeof ClaudeMessageSchema.infer): NormalizedInbound {
    const record = this.asRecord(payload);
    const baseSessionId = this.asString(record.session_id) ?? "default";
    const typeStr = this.asString(record.type);
    ...
    if (typeStr === "assistant" && thought) {
      return { kind: "prompt", sessionId: baseSessionId, text: text ?? "", thought, meta: record };
    }
    ...
    if (command) {
      return { kind: "tool_use", sessionId: baseSessionId, command };
    }
    if (
    ...
    return { kind: "noop", sessionId: baseSessionId, meta: record };
  }

  private normalizeAcpOutbound(payload: AcpEnvelope): NormalizedOutbound {
  ...
  private toGithubEnvelope(normalized: NormalizedOutbound): unknown {
    switch (normalized.kind) {
      case "message_chunk":
        return {
          jsonrpc: "2.0",
      ...
      case "tool_call_update":
        return {
          ...
          params: { toolCallId: normalized.toolCallId, status: normalized.status },
        };
      ...
      case "response":
        return {
          jsonrpc: "2.0",
          id: normalized.requestId ?? Date.now(),
          result: { stopReason: normalized.stopReason ?? "end_turn" },
        };
      case "error":
      ...
      case "noop":
      default:
        return { jsonrpc: "2.0", method: "noop", params: {} };
    }
  }
  ...
  private buildProtocolError(
    requestId: JsonRpcId | string,
    code: number,
    message: string,
  ): AcpResponse {
    return AcpEnvelopeSchema.assert({
      jsonrpc: "2.0",
      id: requestId,
      error: { code, message },
    }) as AcpResponse;
  ...
  private readSessionId(params: unknown): string {
    ...
    return this.asString(record.sessionId) ?? "default";
  }
}
